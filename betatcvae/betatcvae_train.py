...
wandb.init(
    ...
    "pairwise_tc_loss_weight": pairwise_tc_loss_weight,
    }
)
...
if dataset_name == '3dshapes':
    ...
    #val_size = int(np.ceil(0.15 * dataset_size))
    ...
    val_dataset = CelebA(root='data/', split='valid', transform=transform, download=True)
...
model = ControlVAE(img_size, encoder, decoder, latent_dim).cuda()
...
for epoch in range(epochs):
    ...
    for _, (data,label) in enumerate(train_loader):
        ...
        loss = rec_loss_weight * rec_loss + kl_loss_weight * kl_loss + pairwise_tc_loss_weight * pairwise_tc_loss
        ...
        #if i % 100 == 0:
            # print("Epoch ", epoch, ", Iteration ", i, ", Total loss ", loss, ", KL loss ", kl_loss, ", Rec loss ", rec_loss, ", Rec prop loss ", lat_loss_prop_all, ", wwi loss ", pairwise_tc_loss, sep = "")
    print(" Training Epoch ", epoch, ", Epoch Loss ", epoch_loss, sep = "")

    epoch_test_loss, epoch_test_rec_loss, epoch_test_kl_loss, epoch_test_p_tc_loss = evaluate(model, test_loader)
    print("Testing Epoch ", epoch, ", Epoch Loss ", epoch_test_loss, ", Rec loss ", epoch_test_rec_loss,", KL loss ", epoch_test_kl_loss, ", tc loss ", epoch_test_p_tc_loss, sep = "")

     # log metrics to wandb
    wandb.log({"train_loss": epoch_loss, "epoch_test_loss":epoch_test_loss,"epoch_test_rec_loss":epoch_test_rec_loss, "epoch_test_kl_loss":epoch_test_kl_loss, "epoch_test_p_tc_loss":epoch_test_p_tc_loss})
    '''
    if epoch_loss < best_loss:
        best_loss = epoch_loss
        best_weight = model.state_dict()
        best_optimzer = optimizer.state_dict()
        best_epoch = epoch
        best_epoch_rec_loss = epoch_rec_loss
        best_epoch_kl_loss = epoch_kl_loss
        best_epoch_p_tc_loss = epoch_p_tc_loss
        best_epoch_g_tc_loss = epoch_g_tc_loss
        best_epoch_lat_prop_loss = epoch_lat_prop_loss
    '''
    if not os.path.exists(os.path.join('ckpt',dir_name)):
        os.makedirs(os.path.join('ckpt',dir_name))
    if (epoch + 1) % save_epoch_interval == 0:
        torch.save({'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                    }, "ckpt/{}/model{}{}_{}_epoch{}_{}_{}_{}.pt".format(dir_name,model_name, latent_dim, dataset_name, epoch, rec_loss_weight, kl_loss_weight, pairwise_tc_loss_weight))
...
print("==============================================================================================================================")
print("Training Completed Epoch", epoch, ", Total loss: ", mean_epoch_loss, ", Training complete in {:.0f}m {:.0f}s: ".format(time_elapsed // 60, time_elapsed % 60))
#epoch_loss, epoch_rec_loss, epoch_kl_loss, epoch_lat_prop_loss, epoch_p_tc_loss, epoch_g_tc_loss = evaluate(model, test_loader)
#print("Best validation epoch", best_epoch, "loss:",best_loss,"rec_loss:",best_epoch_rec_loss,"kl_loss:",best_epoch_kl_loss,"lat_prop_loss:",best_epoch_lat_prop_loss,\
#      "epoch_p_tc_loss", best_epoch_p_tc_loss, "epoch_g_tc_loss", best_epoch_g_tc_loss, sep=" ")
print("==============================================================================================================================")
...
for _, (data,label) in enumerate(test_loader):
    ...
    if not os.path.exists(os.path.join('vis_reconstruct',dir_name)):
        os.makedirs(os.path.join('vis_reconstruct',dir_name))


    imshow(data[0], f"vis_reconstruct/{dir_name}/"+model_name+str(dataset_name)+str(id)+"_ori.png")
    imshow(reconstruct[0], f"vis_reconstruct/{dir_name}/"+model_name+str(dataset_name)+str(id)+"_recon.png")
