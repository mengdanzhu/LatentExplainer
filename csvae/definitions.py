...
class CSVAE_without_delta_net(nn.Module):
    ...
    def q_zw(self, x, y):
        ...
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # Checks if GPU is available and uses it, else uses CPU
        x = x.to(device)
        y = y.to(device)
        x_flat = x.view(x.size(0), -1)  # Flatten x maintaining the batch size
        #print(x_flat.shape)#[32, 12288]
        #print(y.shape)#[32, 6]
        xy = torch.cat([x_flat, y], dim=1)  # Now x and y have compatible dimensions for concatenation
